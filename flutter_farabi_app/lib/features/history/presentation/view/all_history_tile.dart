...
class AllHistory extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GetCardHistoryCubit, GetCardHistoryState>(
        builder: (context, state) {
      if (state is GetCardHistoryLoading) {
        return const Center(child: CircularProgressIndicator());
      } else if (state is GetCardHistoryError) {
        return Center(child: Text(state.errorString));
      } else if (state is GetCardHistoryLoaded) {
        final historyList = state.data["history"] as List<dynamic>;
        if (historyList.isNotEmpty) {
          for (var historyItem in historyList) {
            String dateAchat = historyItem["dateAchat"] ?? "Date not available";
            String montantAchat =
                historyItem["montantAchat"] ?? "Amount not available";
            String points = historyItem["points"] ?? "Points not available";

            // Print or display the details of each item
            print("Date: $dateAchat, Montant: $montantAchat, Points: $points");
            return DecoratedBox(
                        ...
                        child: Container(
                          ...
                          child: ListTile(
                            ...
                            title: Padding(
                              ...
                              child: Text(
                                "Vous avez gagné $points points fid à $pointDeVente", // Update title text
